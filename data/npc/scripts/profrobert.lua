...
["shamouti"] = 17,
        ...
        if msgcontains(string.lower(msg), 'city') or msgcontains(string.lower(msg), 'cidade') then
           if getPlayerStorageValue(cid, 9658754) == 1 then
              selfSay("Você já escolheu sua cidade iniciante!", cid)
              return true
           else
              selfSay("Você pode escolher sua cidade iniciante entre: {Saffron, Cerulean, Lavender, Fuchsia, Celadon, Viridian, Vermilion, Pewter, Cinnabar, Snow, Ascordbia, Hammlin, Shamouti}.", cid) 
              return true
           end
        elseif places[string.lower(msg)] then
           city = string.lower(msg)
           selfSay("Tem certeza de que você quer começar {".. doCorrectString(msg) .."}?", cid) 
       ...
       elseif msgcontains(msg, "yes") or msgcontains(msg, "sim") and talkState[talkUser] == 2 then   
           if getPlayerStorageValue(cid, 9658754) == 1 then
              ...
              selfSay("OK, então ... Agora sua cidade iniciante é ".. doCorrectString(city)..". Boa sorte em sua jornada!", cid)
