...
table.insert(str, "Holding: "..(yhelds[heldy].name)..". ")
      ...
      -- Calcular o valor do boost
      if boosts > 50 then
          precocertos = ((gastostones[50] * 5) * 400) + ((boosts - 50) * 100000)
      else
          precocertos = ((gastostones[boosts] * 5) * 400)
      end
      -- Obter o nome do Pokémon
      local pokename = getItemAttribute(thing.uid, "poke")
      local pokeCost = pricePokemonsList[pokename] and pricePokemonsList[pokename].price
      if pokeCost then
          if boosts == 0 then
              table.insert(str, "\nPrice: $"..pokeCost)
          else
              table.insert(str, "\nPrice: $"..pokeCost.. " + $"..precocertos.. " [boost +"..boosts.."].")
          end
      else
          table.insert(str, "\nPrice: Desatualizado.")
      end
...
local function getAddonId (looktype)
   ...
   elseif priceItemsList[iname.name] then
      ...
      table.insert(str, ""..iname.description.."\nPrice: $"..calculo..".")
